...
def main():
    ...
    rootMetaComps = []
    for fil in files[1:]:
        root1 = ET.parse(fil).getroot().find('default:com' +
                'ponents', ns).findall('default:component', ns)
        root2 = ET.parse(fil).getroot().find('default:meta' +
                'data', ns).find('default:tools', ns).findall('default:tool', ns)
        root3 = ET.parse(fil).getroot().find('default:meta' +
                'data', ns).findall('default:component', ns)
    ...
    for element in rootTools:
        tree.getroot().find('default:metadata', ns).find('default:' +
                'tools', ns).append(element)
